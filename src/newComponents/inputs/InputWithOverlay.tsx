...
type OverlayField = {
  name: string;
  label: string;
  type: "text" | "number" | "date" | "select" | "numberSelect";
  placeholder?: string;
  required?: boolean;
  helperText?: string;
  options?: InputOption[];
...
type OverlayData = Record<string, OptionValue | OptionValue[] | undefined>;

type InputWithOverlayProps = {
  label?: string;
  value?: OverlayData;
  onChange: (value: OverlayData) => void;
  placeholder?: string;
  defaultValue?: OverlayData | null;
  required?: boolean;
  error?: string;
  helperText?: string;
  className?: string;
...
const InputWithOverlay = ({
  ...
  const handleSaveOverlay = (formData: OverlayData) => {
    onChange(formData);
  ...
  const renderChipsDisplay = () => {
    ...
    const entries = Object.entries(value as OverlayData);
    ...
    <div className={`relative ${width} ${className}`}>
      ...
      <div className="relative">
        <button
          ...
          className={`w-full px-4 py-3 pr-10 theme-input border-2 rounded-xl text-left transition-all cursor-pointer duration-300 focus:outline-none ${
            error
