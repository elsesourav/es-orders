...
import OrderPagesList from "./components/OrderPagesList";
import useOrderData from "./hooks/useOrderData";
import { LOADING_PRODUCT } from "./utils/utils";
...
const OrdersPageView = ({ selectedOrdersState = null }: OrdersPageProps) => {
  const { t } = useLanguage();
  const { stateData, orders, products, resolveProduct, isOrdersLoading } =
  ...
  const checkAndLoadImage = useCallback((url: string) => {
    if (!url) {
      setIsImageLoading(false);
      return;
    }
  ...
  const copySku = useCallback(async (sku: string) => {
